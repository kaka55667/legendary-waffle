<template>
	<view style="height: 100%;">
		<view class="LiuChooseAddress_input" @click="open">
			<text v-if="checkArrText && checkArrText != ''">{{ checkArrText }}</text>
			<text v-else>请选择城市/区/县</text>
		</view>
		<!-- 弹出层 -->
...
export default {
	...
	methods: {
		async setData(province, city) {
			if (province && this.checkBox.length) {
				for (let i in this.checkBox[0]) {
					let item = this.checkBox[0][i]
					if (item.label.includes(province)) {
						let res = await this.check(i)

						if (city && this.checkBox.length > 1) {
							for (let ii in this.checkBox[1]) {
								let item2 = this.checkBox[1][ii]
								if (item2.label.includes(city)) {
									let res = await this.check(ii)
								}
							}
						}
		...
		async check(index) {
			...
			console.log('this.checkArr', this.checkArr, this.checkBox);
		...
		async getData() {
			...
			if (this.checkArr.length) {
				...
				list = this.cityList.filter(e => {
					return this.disableArea.indexOf(e.label) == -1
				});
				uni.hideLoading()
				this.$set(this.checkBox, this.id, list)
			}
